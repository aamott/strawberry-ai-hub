...
from .prompt import DEVICE_AGNOSTIC_KEY
...
class HubSkillService:
    ...
    async def get_system_prompt(
        ...
        role section + tools section + device keys footer.

        Custom prompts from ``settings.system_prompt`` are passed
        through with legacy tool sections automatically stripped.
        """
        from .prompt import build_system_prompt as build_hub_prompt

        devices = await self.devices._get_user_devices()

        # Build filtered device key list — exclude hub from
        # non-hub requests since it's a control plane.
        filtered_device_keys: list[str] = [DEVICE_AGNOSTIC_KEY]
        ...
        # Custom prompt override (stripped of legacy tool sections)
        custom = (
