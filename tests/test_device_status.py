...
async def test_device_status_lifecycle(client, db_session):
    ...
    response = await client.post("/api/users/setup", json={"username": "testuser", "password": "password"})
    if response.status_code == 400:
        await client.post("/api/users/login", json={"username": "testuser", "password": "password"})
    
    response = await client.post("/api/users/login", json={"username": "testuser", "password": "password"})
    assert response.status_code == 200
    user_token = response.json()["access_token"]
    user_auth_headers = {"Authorization": f"Bearer {user_token}"}
    
    response = await client.post("/api/devices/token", json={"name": "Test Device"}, headers=user_auth_headers)
    assert response.status_code == 200
