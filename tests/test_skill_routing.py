...
@pytest.mark.asyncio
async def test_search_skills_prioritizes_connected_devices(mock_db_session, mock_connection_manager):
    ...
    device_disconnected = Device(id="d1", name="spoke two", user_id="u1", is_active=True)
    device_connected = Device(id="d2", name="strawberry spoke", user_id="u1", is_active=True)
    
    # Mock DB returning devices
    mock_db_session.execute.return_value.scalars.return_value.all.return_value = [device_disconnected, device_connected]
    
    # Mock ConnectionManager: only "strawberry spoke" (d2) is connected
    mock_connection_manager.is_connected.side_effect = lambda device_id: device_id == "d2"
    ...
    # Actually, _get_user_devices caches the result, so we only need to handle the first call? 
    ...
    devices_result.scalars.return_value.all.return_value = [device_disconnected, device_connected]
    ...
    # IMPORTANT: This assertion will FAIL before the fix, because it sorts alphabetically
    # "spoke two" vs "strawberry spoke" -> "spoke two" wins if we don't prioritize connection
    # Or actually, "strawberry spoke" vs "spoke two". "spoke two" is alphabetically first? 
    # "s", "p", "o", "k", "e", " " "t"...
    # "s", "t", "r", "a", "w"...
